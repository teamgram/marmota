...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		t.signal.Reset(d)
		if Debug {
			logx.Infof("timer: add reset delay %d ms", int64(d)/int64(itime.Millisecond))
		}
	}
	if Debug {
		logx.Infof("timer: push item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
}
...
func (t *Timer) del(td *TimerData) {
	...
	if i < 0 || i > last || t.timers[i] != td {
		...
		if Debug {
			logx.Infof("timer del i: %d, last: %d, %p", i, last, td)
		}
		return
	}
	if i != last {
	...
	if Debug {
		logx.Infof("timer: remove item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
}
...
func (t *Timer) expire() {
	...
	for {
		if len(t.timers) == 0 {
			d = infiniteDuration
			if Debug {
				logx.Infof("timer: no other instance")
			}
			break
		}
		td = t.timers[0]
		...
		t.lock.Unlock()
		if fn == nil {
			logx.Errorf("expire timer no fn")
		} else {
			if Debug {
				logx.Infof("timer key: %s, expire: %s, index: %d expired, call fn", td.Key, td.ExpireString(), td.index)
			}
			fn()
		}
		t.lock.Lock()
	...
	t.signal.Reset(d)
	if Debug {
		logx.Infof("timer: expier reset delay %d ms", int64(d)/int64(itime.Millisecond))
	}
	t.lock.Unlock()
}
