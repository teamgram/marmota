...
func (lru *LRUCache) Get(key string) (v Value, ok bool) {
	...
	return element.Value.(*entry2).value, true
}
...
func (lru *LRUCache) Delete(key string) bool {
	...
	lru.size.Add(-element.Value.(*entry2).size)
	return true
}
...
func (lru *LRUCache) Stats() (length, size, capacity, evictions int64, oldest time.Time) {
	...
	if lastElem := lru.list.Back(); lastElem != nil {
		oldest = lastElem.Value.(*entry2).timeAccessed
	}
	return int64(lru.list.Len()), lru.size.Load(), lru.capacity, lru.evictions.Load(), oldest
}
...
func (lru *LRUCache) Keys() []string {
	...
	for e := lru.list.Front(); e != nil; e = e.Next() {
		keys = append(keys, e.Value.(*entry2).key)
	}
	return keys
}
...
func (lru *LRUCache) updateInplace(element *list.Element, value Value) {
	...
	sizeDiff := valueSize - element.Value.(*entry2).size
	element.Value.(*entry2).value = value
	element.Value.(*entry2).size = valueSize
	lru.size.Add(sizeDiff)
	lru.moveToFront(element)
	lru.checkCapacity()
}
...
func (lru *LRUCache) moveToFront(element *list.Element) {
	...
	// timeAccessed is only set in addNew (for Oldest stats); skip time.Now() on Get to avoid hot-path cost
}

func (lru *LRUCache) addNew(key string, value Value) {
	newEntry := &entry2{key, value, int64(value.Size()), time.Now()}
	element := lru.list.PushFront(newEntry)
	lru.table[key] = element
	lru.size.Add(newEntry.size)
	lru.checkCapacity()
}

func (lru *LRUCache) checkCapacity() {
	// Partially duplicated from Delete
	for lru.size.Load() > lru.capacity {
		delElem := lru.list.Back()
		delValue := delElem.Value.(*entry2)
		lru.list.Remove(delElem)
		delete(lru.table, delValue.key)
		lru.size.Add(-delValue.size)
		lru.evictions.Add(1)
	}
}
