...
func (c *TestTcpPingClient) OnNewClient(client *TcpClient) {
	log.Info("client OnNewConnection %v", client.GetRemoteName())
	c.ready <- true
}

func (c *TestTcpPingClient) OnClientDataArrived(client *TcpClient, msg interface{}) error {
	log.Info("client OnClientDataArrived - client: %s, receive data: %v", client.GetRemoteName(), msg)
	c.receiver <- msg
	return nil
}

func (c *TestTcpPingClient) OnClientClosed(client *TcpClient) {
	log.Info("client OnConnectionClosed %s", client.GetRemoteName())
}
