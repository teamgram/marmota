...
func (c *TestTcpPingClientGroup) OnNewClient(client *TcpClient) {
	log.Info("client group OnNewConnection %v", client.GetRemoteName())
	c.ready.Add(1)
}

func (c *TestTcpPingClientGroup) OnClientDataArrived(client *TcpClient, msg interface{}) error {
	log.Info("client group OnClientDataArrived - client: %s, receive data: %v", client.GetRemoteName(), msg)
	c.receiver <- msg
	return nil
}

func (c *TestTcpPingClientGroup) OnClientClosed(client *TcpClient) {
	log.Info("client group OnConnectionClosed %s", client.GetRemoteName())
}

func (c *TestTcpPingClientGroup) OnClientTimer(client *TcpClient) {
	log.Info("client group OnTimer")
}
